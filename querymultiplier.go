...
looper := int(math.Ceil(float64(*maximum-*minimum)/float64(*rows)))

    for i := 0; i < looper; i++ {
        if ( *keytype == "LIMIT" ) || ( *keytype == "limit") {
                if ( i == looper-1) {
                        fmt.Printf("%s LIMIT %d,%d;\n", *querystring, (*rows*i)+*minimum, *maximum-((*rows*i)+*minimum))
                } else {
                        fmt.Printf("%s LIMIT %d,%d;\n", *querystring, (*rows*i)+*minimum, *rows) 
                }
        } else {
                if ( i == looper-1) {
                        fmt.Printf("%s WHERE %s between %d and %d;\n", *querystring, *keytype, (*rows*i)+*minimum, *maximum)
                } else {
                        fmt.Printf("%s WHERE %s between %d and %d;\n", *querystring, *keytype, (*rows*i)+*minimum, *rows*(i+1)+*minimum)
                }
        }
        }
}
